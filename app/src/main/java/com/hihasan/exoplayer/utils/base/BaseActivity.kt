...
open class BaseActivity : AppCompatActivity() {
    ...
    //show single toast
    open fun showSingleToast( context: Context?,msg: String?){
        val duration = Toast.LENGTH_SHORT
        if (toast == null)
            toast = Toast.makeText(context, msg, duration)
        else
            toast!!.setText(msg) // smoother transition than cancel + new toast
        toast!!.show()
    ...
    protected open fun downloadImage(url: String?): String? {
        ...
        if (url != null) {
            ...
            val file =
                File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).path + EXO_PLAYER + filename)
            path = file.path
            if (file.exists()) {
            } else {
